...
def spike_td3(env_name, ac_kwargs=dict(),seed=0, tb_comment='', model_idx=0,network='deepAC',start_timesteps=25e3,
...
if __name__ == "__main__":
	...
	parser.add_argument('--num_model', type=int, default=1)  # 多个模型/多个随机种子 下运行  结果取平均           预训练 在一个环境上、一个seed下、聚类一个中心得到 四个动力学参数

	parser.add_argument('--encoder_pop_dim', type=int, default=10)
	parser.add_argument('--to_spike', type=str, default='none')
	...
	for num in range(args.start_model_idx, args.start_model_idx + args.num_model): # 把多个模型的循环放在py脚本内
		...
		spike_td3(args.env_name, ac_kwargs=AC_KWARGS,
