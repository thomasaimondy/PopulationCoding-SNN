...
class PopSpikeEncoderUniformSpike(PopSpikeEncoderRegularSpike):
    """ Learnable Population Coding Spike Encoder with Poisson Spike Trains """
    def __init__(self, obs_dim, pop_dim, spike_ts, mean_range, std, device):
        """
        :param obs_dim: observation dimension
...
class SpikeActor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim,max_action, hidden_sizes=[256, 256],encoder_pop_dim=10, decoder_pop_dim=10,
                 mean_range=(-3,3), std=math.sqrt(0.15), spike_ts=10, device=torch.device('cpu'), to_spike='none',ntype='LIF'):
        """
        :param obs_dim: observation dimension
        :param act_dim: action dimension
        ...
        if to_spike=='poisson':
            self.encoder = PopSpikeEncoderPoissonSpike(state_dim, encoder_pop_dim, spike_ts, mean_range, std, device)
        elif to_spike=='regular':
            self.encoder = PopSpikeEncoderRegularSpike(state_dim, encoder_pop_dim, spike_ts, mean_range, std, device)
        elif to_spike=='uniform':
            self.encoder = PopSpikeEncoderUniformSpike(state_dim,encoder_pop_dim,spike_ts,mean_range,std,device)
        elif to_spike=='none':
